export type RmdRow = { age: number; distributionPeriod: number };

// IRS Uniform Lifetime Table (2022+). Ages 72–120.
// Source: IRS Pub 590-B.
export const UNIFORM_LIFETIME_TABLE: RmdRow[] = [
...
export function getUniformLifetimeDivisor(age: number) {
  return UNIFORM_LIFETIME_TABLE.find((r) => r.age === age)?.distributionPeriod ?? null;
}

export function computeRmd(age: number, assets: number) {
  const divisor = getUniformLifetimeDivisor(age);
  if (!divisor || assets <= 0) return { divisor, rmd: null as number | null };
  return { divisor, rmd: assets / divisor };
}
