...
export function EmailResizerTool() {
  ...
  async function run(direction: "shorter" | "longer") {
    ...
    try {
      ...
      const subject = json?.resized.subject ? `Subject: ${json.resized.subject}\n\n` : "";
      setResult(subject + (json?.resized.body ?? ""));
    } catch (e) {
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <div className="rounded-3xl border border-white/10 bg-zinc-900/40 p-6">
        ...
        <textarea
          ...
          className="mt-4 h-80 w-full resize-none rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm leading-6 text-zinc-50 outline-none focus:border-teal-400/60"
        />
      ...
      <div className="rounded-3xl border border-white/10 bg-zinc-900/40 p-6">
        ...
        <textarea
          value={result}
          readOnly
          placeholder="Resized email will appear here…"
          className="mt-4 h-80 w-full resize-none rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm leading-6 text-zinc-50 outline-none"
        />
        <div className="mt-3 text-xs text-zinc-400">
          Scaffold mode (heuristic). Add <code>OPENAI_API_KEY</code> later for true tone-preserving rewriting.
        </div>
      </div>
    </div>
  );
